...
let busyMap=false;
async function request(path,method='GET',body){const r=await fetch(path,{method,cache:'no-store',headers:body?{'Content-Type':'application/json'}:{},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(12000)});const b=await r.json();if(!r.ok){const e=new Error(b.error||'No se pudo completar la operación.');e.status=r.status;if(r.status===401&&recoveryKey){backup();aviso('La sesión venció. Tus cambios se conservaron; volvé a ingresar.',true);const link=document.createElement('a');link.href='login.html';link.textContent=' Volver a ingresar';$('#ed-aviso').append(link);}throw e;}return b;}
...
if(!session.editor){location.replace('login.html');$('#ed-acceso').innerHTML='<p>El mapa es público. Para editar necesitás una cuenta autorizada por la organización.</p><a class="btn-pri" href="login.html">Ingresar al editor</a>';}
else{
  ...
  $('#ed-logout').onclick=async()=>{if(dirty||serverUnavailable){if(!await confirmar('Cerrar sesión','<p>Hay cambios sin guardar en el servidor. Descargá un respaldo antes de salir.</p>','Salir de todas formas'))return;}clearTimeout(saveTimer);await saving.catch(()=>{});await request('api/auth/logout','POST',{});try{localStorage.removeItem(recoveryKey);}catch{}location.replace('login.html');};
